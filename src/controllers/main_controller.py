...
class MainBrowserController:
    ...
    async def prepare_page(self, page: Page, options):
        try:
            ...
            if options.wait_for_network == 'idle':
                await self.navigation_controller.wait_for_network_idle(page, options.wait_for_timeout)
            elif options.wait_for_network == 'mostly_idle':
                await self.navigation_controller.wait_for_network_mostly_idle(page, options.wait_for_timeout)

            if options.custom_js:
                await self.content_controller.execute_custom_js(page, options.custom_js)

            if options.wait_for_selector:
                await self.navigation_controller.wait_for_selector(page, options.wait_for_selector, options.wait_for_timeout)
    ...
    async def goto_with_timeout(self, page: Page, url: str, timeout: float = 5.0):
        return await self.navigation_controller.goto_with_timeout(page, url, timeout)
    ...
    async def wait_for_dynamic_content(self, page: Page, check_interval: int = 1000, max_unchanged_checks: int = 5):
        return await self.content_controller.wait_for_dynamic_content(page, check_interval, max_unchanged_checks)